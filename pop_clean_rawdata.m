...
if nargin < 2
    ...
    uilist =    {...
        ...
        ...
        {} {'style' 'checkbox' 'string' 'Max acceptable std. dev. (uV)' 'value' 1 'tag' 'rmnoise' 'userdata' 'chan' } ...
        ...
        ...
        {} {'style' 'checkbox' 'string' 'Perform ASR bad burst correction' 'fontweight' 'bold' 'value' 1 'tag' 'asr' 'callback' cb_asr } ...
        ...
        ...
        {} {'style' 'checkbox' 'string' 'Remove irrecoverable data periods' 'fontweight' 'bold' 'value' 1 'tag' 'rejwin' 'callback' cb_rej } ...
        {} {'style' 'text' 'tag' 'asrwintext' 'string' 'Acceptable [min max] channel power limits (std. dev.)'  'userdata' 'rej'} ...
        ...
        {} {'style' 'text' 'tag' 'asrwintext' 'string' 'Maximum out-of-range channels (%)'  'userdata' 'rej'} ...
        {'style' 'edit' 'string' '0.25','tag', 'rejwinval2', 'userdata' 'rej' 'tooltipstring', wordwrap('If a time window has a larger fraction of simultaneously corrupted channels than this (after the other cleaning attempts), it will be cut out of the data. This can happen if a time window was corrupted beyond the point where it could be recovered.',80)} ...
        ...
        {} {'style' 'checkbox' 'string' 'Pop up data scrolling window with rejected data highlighted' 'tag' 'vis' 'value' fastif(length(EEG) > 1, 2, 1) 'enable' fastif(length(EEG) > 1, 'off', 'on') }};

    row   = [0.1 1 0.3];
    row2  = [0.1 1.2 0.1];
    geom =     { 1 row     1   1 row     row     row     1   1 row     row2    1   1 row  row     1   1 };
    geomvert = [ 1 1       0.3 1 1       1       1       0.3 1 1       1       0.3 1 1    1       0.3 1 ];
    ...
    if outs.rejwin
        ...
        opt.WindowCriterion = str2num(outs.rejwinval2);
    end
